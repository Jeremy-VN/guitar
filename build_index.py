#!/usr/bin/env python3
"""Regenerate index.html (the contents page) from lessons.json.
...
for l in lessons:
    ...
    cards.append(f'''    <a class="card" href="{esc(l["file"])}">
...
count = len(lessons)
subtitle = f"{count} lesson{'s' if count != 1 else ''}"
...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .date{{display:inline-block;font-size:12.5px;font-weight:600;color:var(--accent);
...
<div class="wrap">
  <header>
    ...
    <p class="sub">{subtitle} &middot; tap a card to open</p>
  </header>

{cards_html}

  <footer>Each lesson is a self-contained page with audio references built in.</footer>
</div>
</body>
</html>
'''
(here / "index.html").write_text(PAGE, encoding="utf-8")
print(f"Wrote index.html with {count} lesson card(s).")
